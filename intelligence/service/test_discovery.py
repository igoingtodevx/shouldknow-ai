...
class DiscoveryParserTests(unittest.TestCase):
    ...
    def test_extracts_matching_tool_links(self):
        markdown = '[Alpha](https://example.test/tool/alpha) [Beta](/tool/beta)'
        ...
        self.assertEqual(candidates[1]['url'], 'https://example.test/tool/beta')
    ...
    def test_strips_image_prefix_and_tracking_query(self):
        markdown = '[Image: Delta](https://example.test/tool/delta?utm_source=x#hero)'
        candidates = extract_directory_candidates(markdown, self.source)
        self.assertEqual(candidates[0]['title'], 'Delta')
        self.assertEqual(candidates[0]['url'], 'https://example.test/tool/delta')
